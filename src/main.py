...
def load_summary(youtube_video_link: str) -> str | None:
    ...
    summary_path = root_dir / "video_summary.md"
...
def load_video_link_from_config(config_path: str | None = None) -> str:
    """Đọc đường dẫn video YouTube từ file cấu hình."""
    if config_path is None:
        config_path = os.path.join(root_dir, "config", "video_config.txt")
    try:
...
if __name__ == "__main__":
    youtube_video_link = load_video_link_from_config()
    video_id: str = ""
    try:
